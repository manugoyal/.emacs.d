...
** Minibuffer completion
   We use Icomplete mode to view and cycle between different possible minibuffer
   completions. More complex stuff like helm and ido are nice, but tend to get
   *REALLY* slow when editing over remote connections, so we keep it simple.
   #+begin_src emacs-lisp
     (icomplete-mode 1)
   #+end_src
** Buffer menu
