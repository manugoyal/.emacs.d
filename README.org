...
** Shell Environment variables
   ...
   #+begin_src emacs-lisp
     ...
     (require 's)
     (let (
           (env (shell-command-to-string "printenv"))
           (getvarname (lambda (line)
                         (let ((ind (s-index-of "=" line)))
                           (if ind (s-left ind line) line))))
           )
       (exec-path-from-shell-copy-envs
        (map 'list getvarname (s-lines env)))
       )
...
** Recentf mode
   ...
   #+end_src
** Paredit mode
   Add paredit to the programming modes
   #+begin_src emacs-lisp
